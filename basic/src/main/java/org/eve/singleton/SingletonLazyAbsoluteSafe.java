...
/**
 * 单例-懒汉模式，绝对线程安全
 * instance = new SingletonLazyAbsoluteSafe()，是两步操作：对象初始化、变量赋值
 ...
 */
public class SingletonLazyAbsoluteSafe {

    private volatile static SingletonLazyAbsoluteSafe instance;

    private SingletonLazyAbsoluteSafe() {
    }

    public static SingletonLazyAbsoluteSafe getInstance() {
        if (null == instance) {
            synchronized (SingletonLazyAbsoluteSafe.class) {
                if (null == instance) {
                    instance = new SingletonLazyAbsoluteSafe();
                }
            }
        }
        return instance;
